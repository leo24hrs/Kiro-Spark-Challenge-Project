...
import type { ArenaResult, Severity, TimelineEntry, Vulnerability } from './types.js';
...
async function main() {
  ...
  const allVulns: Vulnerability[] = scanReports.flatMap(r =>
    r.vulnerabilities.map(v => ({ ...v, file_path: shortPath(r.file_path) }))
  );
  ...
  try {
    if (shouldRunArena) {
      ...
      const arenaCallbacks = {
        ...
        onGladiatorStart: (id: number, name: string) => {
          server.pushGladiator({ id, name, status: 'fighting' });
          server.pushLog({ t: timestamp(startedAt), msg: `${name} enters the arena…`, cls: 'dim' as const });
        },
        onGladiatorDone: (id: number, name: string, gladResult: import('./types.js').GladiatorResult) => {
      ...
      const arenaBundle = await forgeTrial(
        server,
        startedAt,
        allVulns,
        arenaResult.gladiator_results,
        arenaResult.casualties.length === 0,
      );
      ...
      result = {
        ...
        overall_severity: highestSeverity as Severity,
        gladiator_results: [],
        survivors: [],
        casualties: [],
        vulnerabilities: allVulns,
...
function computeHighestSeverity(vulns: Vulnerability[]): Severity {
 ...
 */
async function forgeTrial(
  server: ReturnType<typeof startArenaServer>,
  startedAt: number,
  vulnerabilities: Vulnerability[],
  gladiators: import('./types.js').GladiatorResult[],
  passed: boolean,
): Promise<import('./learn/mcq-generator.js').MCQBundle> {
