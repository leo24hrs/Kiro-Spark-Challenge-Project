...
server.tool(
  'create_shadow_clone',
  'Creates an isolated Neon DB branch (shadow clone) for chaos simulation.',
  {
...
server.tool(
  'run_gladiator',
  'Dispatches a single Gladiator sub-agent to attack the shadow clone.',
  {
...
server.tool(
  'generate_scoreboard',
  'Aggregates Gladiator results into a gamified ASCII-art scoreboard report.',
  {
    gladiator_results: z.array(
      z.object({
...
server.tool(
  ...
  'Destroys a Neon DB branch shadow clone after simulation is complete.',
  {
    branch_id: z.string(),
  },
  async ({ branch_id }) => {
 ...
 */
function fallbackResult(gladiator_id: number, err: unknown): GladiatorResult {
  return {
...
async function orchestrate(
  ...
  const clone = await createShadowClone(
    connection_string,
    'colosseum-' + Date.now(),
  );
...
server.tool(
  'run_colosseum',
  'Runs the full Colosseum pipeline: creates a shadow clone, executes the SQL, unleashes all 5 Gladiators in parallel, renders the scoreboard, and destroys the clone.',
  {
    connection_string: z.string(),
    sql_command: z.string(),
    developer_id: z.string(),
...
server.tool(
  'scan_file',
  'Scans a source file for security vulnerabilities using five parallel Code Gladiator scanners. Returns a ScanReport with all findings and a gamified markdown report.',
  {
    file_path: z.string().describe('Path of the file being scanned'),
    file_content: z.string().describe('Full text content of the file to scan'),
  },
  async ({ file_path, file_content }) => {
    const report = await scanFile(file_path, file_content);
